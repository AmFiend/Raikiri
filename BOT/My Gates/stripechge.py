...
# Configuration
# -------------------------------------------------------------
GATE_NAME = "Stripe Charge $1"
API_BASE = "http://147.93.53.240:5020/check"
MAX_MSC_LIMIT = 10
MAX_TSC_LIMIT = 100
        ...
        for attempt in range(2):
            try:
                ...
                if "Approved" in status:
                    return "Approved ✅", f"{message} ({price})", gate
                elif "Declined" in status:
                    return "Declined ❌", f"{message} ({price})", gate
                else:
                    return status, message, gate
            except Exception:
                if attempt == 1:
