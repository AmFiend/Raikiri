...
async def stripe_charge_cmd(Client, message):
    try:
        ...
        if getcc == False:
            resp = f"〈<a href='tg://user?id={user_id}'>{first_name}</a>〉-» PayPal 1$ charge - CHECK\n\n〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» PayPal 1$ charge \n\n<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /pp1 cc|month|year|cvc"
...
async def stripe_mass_check_cmd(Client, message):
    try:
        ...
        if not ccs:
            resp = f"〈<a href='tg://user?id={user_id}'>{first_name}</a>〉-» PayPal 1$ charge - MASS\n\n〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» PayPal 1$ charge \n\n<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» Reply to cards or /mpp1 cc|mm|yy|cvc (up to {MAX_MSC_LIMIT})"
            await message.reply_text(resp, quote=True)
            return
...
async def stripe_txt_check_cmd(Client, message):
    try:
        ...
        if not target_message or not target_message.document.file_name.endswith('.txt'):
            resp = f"〈<a href='tg://user?id={user_id}'>{first_name}</a>〉-» PayPal 1$ charge - TXT\n\n〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» PayPal 1$ charge \n\n<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» Upload a .txt file with /tsc caption or reply to a .txt file with /tpp1 (up to {MAX_TSC_LIMIT})"
            await message.reply_text(resp, quote=True)
            return
...
"""
    ...
    header_text = f"<b>↯ PayPal 1$ charge 💎\nNumber Of CC Check : [{len(ccs)}]\n</b>\n"
