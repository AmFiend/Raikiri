...
COOKING_GIF = "https://files.manuscdn.com/user_upload_by_module/session_file/310519663526340909/nNsBhlEvRwguvumk.gif"
...
@Client.on_message(filters.command("ar", [".", "/"]))
async def arcenus_cmd(Client, message):
    try:
        user_id = str(message.from_user.id)
        first_name = message.from_user.first_name
        checkall = await check_all_thing(Client, message)

        gateway = "Arcenus 1.0$ 🛡️"
        ...
        if getcc == False:
            resp = f"〈<a href='tg://user?id={user_id}'>{first_name}</a>〉-» Arcenus 1.0$ - CHECK\n\n〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» {gateway} \n\n<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /ar cc|month|year|cvc"
            await message.reply_text(resp, quote=True)
            return

        cc, mes, ano, cvv = getcc[0], getcc[1], getcc[2], getcc[3]
        fullcc = f"{cc}|{mes}|{ano}|{cvv}"
        endpoint_url = f"https://onyxenvbot.up.railway.app/arcenus/key=yashikaaa/cc={fullcc}"
