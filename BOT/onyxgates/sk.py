...
@Client.on_message(filters.command("skb", [".", "/"]))
async def sk_cmd(Client, message):
    try:
        ...
        if getcc == False:
            resp = f"〈<a href='tg://user?id={user_id}'>{first_name}</a>〉-» SK Based 10$ - CHECK\n\n〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» {gateway} \n\n<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /skb cc|month|year|cvc"
            await message.reply_text(resp, quote=True)
            return

        cc, mes, ano, cvv = getcc[0], getcc[1], getcc[2], getcc[3]
        fullcc = f"{cc}|{mes}|{ano}|{cvv}"
        endpoint_url = f"https://onyxenvbot.up.railway.app/sk/key=yashikaaa/cc={fullcc}"
