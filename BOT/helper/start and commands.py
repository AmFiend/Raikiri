...
async def send_colored_msg(client, chat_id, text, reply_markup=None, is_edit=False, message_id=None, parse_mode="HTML"):
    bot_token = getattr(client, "bot_token", None)
    if not bot_token:
        if is_edit: return await client.edit_message_text(chat_id, message_id, text, reply_markup=reply_markup)
        return await client.send_message(chat_id, text, reply_markup=reply_markup)

    method = "editMessageText" if is_edit else "sendMessage"
...
async def safe_edit(client, chat_id, message_id, text, reply_markup=None):
    await send_colored_msg(client, chat_id, text, reply_markup=reply_markup, is_edit=True, message_id=message_id)
...
async def button_callback(client, callback_query):
    ...
    if query.data == "register":
        ...
        await send_colored_msg(client, query.message.chat.id, resp, reply_markup=InlineKeyboardMarkup(keyboard), is_edit=True, message_id=query.message.id)
        ...
        await send_colored_msg(client, query.message.chat.id, message, reply_markup=InlineKeyboardMarkup(keyboard), is_edit=True, message_id=query.message.id)
        ...
        try:
            ...
            if video_file in VIDEO_FILE_IDS:
                ...
                await send_colored_msg(client, query.message.chat.id, original_message, reply_markup=reply_markup, is_edit=True, message_id=query.message.id, parse_mode="Markdown")
        except Exception as e:
